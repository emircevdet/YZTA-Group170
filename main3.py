...
class EhliyetGoApp:
    ...
    def start_topic_test(self, topic_id):
        ...
        self.show_test(questions, f"Konu Testi", lambda score, total: self.finish_topic_test(topic_id, score, total))
    
    def show_test_screen(self, questions, finish_callback):
        """Test arayüzünü gösterir"""
        self.show_test(questions, "Seviye Belirleme Testi", finish_callback)
    ...
    def show_question(self):
        ...
        # Soru numarası ve ilerleme - Daha kompakt
        ...
        # Cevap seçenekleri - Daha kompakt tasarım
        ...
        for i, option in enumerate(options):
            ...
            def on_enter(event, frame=option_frame):
                frame.config(bg="#e9ecef", highlightbackground="#0077cc")
            
            def on_leave(event, frame=option_frame):
                if self.selected_answer.get() != chr(65 + i):
            ...
            # Seçenek içeriği - Padding azaltıldı
            ...
            # Seçenek metni - Font boyutu küçültüldü
            ...
            # Radio button (gizli, sadece işlevsellik için)
            radio = tk.Radiobutton(content_frame, variable=self.selected_answer, 
                                  value=chr(65 + i), bg="#f8f9fa", 
                                  activebackground="#f8f9fa", relief="flat",
                                  highlightthickness=0, bd=0)
            radio.pack(side="right")
            
            self.option_frames.append((option_frame, content_frame, letter_label, option_label, radio))

        # Alt kısım - Butonlar - Sabit pozisyon
        bottom_frame = tk.Frame(self.question_frame, bg="white")
        bottom_frame.pack(fill="x", side="bottom", pady=(10, 0))
        
        # Butonlar - Sabit konumlandırma
        button_container = tk.Frame(bottom_frame, bg="white")
        button_container.pack(fill="x")
        
        # Sol taraf - Önceki butonu
        ...
        if self.current_question > 0:
            ...
            # Hover efekti
            def on_enter_prev(e):
                prev_btn.config(bg="#5a6268")
            def on_leave_prev(e):
                prev_btn.config(bg="#6c757d")
            
            prev_btn.bind("<Enter>", on_enter_prev)
            prev_btn.bind("<Leave>", on_leave_prev)
        
        # Sağ taraf - Sonraki/Bitir butonu
        ...
        if self.current_question < len(self.test_questions) - 1:
            ...
            # Hover efekti
            def on_enter_next(e):
                next_btn.config(bg="#005bb5")
            def on_leave_next(e):
                next_btn.config(bg="#0077cc")
            ...
            def on_enter_finish(e):
                finish_btn.config(bg="#219a52")
            def on_leave_finish(e):
                finish_btn.config(bg="#27ae60")
            
            finish_btn.bind("<Enter>", on_enter_finish)
            finish_btn.bind("<Leave>", on_leave_finish)
    ...
    def update_option_styles(self):
        ...
        for i, (frame, content, letter, option, radio) in enumerate(self.option_frames):
    ...
    def show_profile(self):
        ...
        # Son testler bölümü
        recent_frame = tk.Frame(profile_box, bg="white", padx=20, pady=15)
        recent_frame.pack(fill="x")
        
        tk.Label(recent_frame, text="📝 Son Testler", 
                 font=("Arial", 16, "bold"), bg="white", fg="#222").pack(anchor="w", pady=(0, 10))
        
        # Son 3 test sonucu
        try:
            self.cursor.execute('''
                SELECT tr.score, tr.total_questions, tr.test_date, t.name
                ...
                LIMIT 3
            ''', (self.current_user.get('id', 0),))
            recent_tests = self.cursor.fetchall()
            
            if recent_tests:
                for test in recent_tests:
                    score, total, date, topic_name = test
                    percentage = round((score / total) * 100, 1) if total > 0 else 0
                    topic_display = topic_name if topic_name else "Seviye Testi"
                    
                    test_row = tk.Frame(recent_frame, bg="#f8f9fa", relief="solid", bd=1)
                    test_row.pack(fill="x", pady=2, padx=5)
                    
                    tk.Label(test_row, text=f"{topic_display}: %{percentage} ({score}/{total})", 
                             font=("Arial", 11), bg="#f8f9fa", fg="#333", anchor="w").pack(side="left", padx=10, pady=5)
            else:
                tk.Label(recent_frame, text="Henüz test çözmediniz.", 
                         font=("Arial", 11), bg="white", fg="#999", anchor="w").pack()
                
        except Exception as e:
            print(f"Son testler sorgusu hatası: {e}")
            tk.Label(recent_frame, text="Test geçmişi yüklenemedi.", 
                     font=("Arial", 11), bg="white", fg="#999", anchor="w").pack()
