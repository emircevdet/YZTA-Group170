...
class DriveMate:
    def __init__(self, root):
        ...
        auto_login_success = self.check_auto_login()
        
        # Eğer otomatik giriş başarısızsa ana menüyü göster
    ...
    def setup_gemini_api(self):
        ...
        if not API_KEY:
            messagebox.showerror("Hata", "API anahtarı bulunamadı! Lütfen .env dosyasını kontrol edin.")
            return
    ...
    def check_auto_login(self):
        """Kayıtlı kullanıcı bilgilerini kontrol eder ve otomatik giriş yapar"""
        try:            
            import os
            import json

            # Kullanıcı verilerini saklayacağımız dosya yolu
            user_data_file = os.path.join(os.path.expanduser("~"), ".drivemate_user.json")
            ...
            if os.path.exists(user_data_file):
                ...
                if email and password_hash:
                    ...
                    if user:    
                        ...
                        # Kullanıcı seviyesini de yükle
                        self.load_user_level()
                        
                        # Otomatik girişte de hoş geldin pop-up'ını göster
                        self.show_welcome_popup(user[1])
                        
                         # Pop-up kapandıktan sonra ana menüyü göstermek için delay ekle
        ...
        return False  # Otomatik giriş başarısız    

    def load_user_level(self):
        """Kullanıcının seviyesini yükler"""
        try:
            self.cursor.execute('''
            ...
            print(f"Seviye yükleme hatası: {e}")    

    def save_user_credentials(self, email, password_hash):
        """Kullanıcı bilgilerini dosyaya kaydeder"""
        try:
            import os
            import json
    ...
    def clear_saved_credentials(self):
        """Kayıtlı kullanıcı bilgilerini siler"""
        try:
            import os
            user_data_file = os.path.join(os.path.expanduser("~"), ".drivemate_user.json")
            if os.path.exists(user_data_file):
            ...
            print(f"Kullanıcı bilgileri silme hatası: {e}")                
    ...
    def show_login(self):
        ...
        form_box = tk.Frame(self.root, bg="white", bd=0, relief="flat", highlightthickness=0)
        form_box.pack(pady=10)
        form_box.place(relx=0.5, rely=0.5, anchor="center", width=380, height=320)
        form_box.configure(highlightbackground="#ddd", highlightcolor="#ddd")
        form_box.pack_propagate(False)
        ...
        remember_frame = tk.Frame(form_box, bg="white")
        remember_frame.pack(fill="x", padx=30, pady=(0, 10))
        ...
        def login():
            ...
            if user:
                ...
                if remember_var.get():
                    self.save_user_credentials(email, hashed_password)

                # Hoş geldin pop-up'ını göster
                self.show_welcome_popup(user[1])
                 # Ana menüyü hemen gösterme, pop-up kapandıktan sonra göster
            else:
    ...
    def show_welcome_popup(self, email):
        """Hoş geldin pop-up'ını gösterir"""
        ...
        welcome_window.geometry(f"500x350+{x}+{y}")

        # Pop-up kapandığında ana menüyü göster
        def on_close():
            welcome_window.destroy()
            self.show_main_menu()
            
        # Modern kart tasarımı
        card_frame = tk.Frame(welcome_window, bg="white", relief="flat", bd=0)
        card_frame.pack(fill="both", expand=True, padx=30, pady=30)
        
        # Hoş geldin ikonu
        welcome_icon = tk.Label(card_frame, text="🎉", font=("Arial", 42), bg="white")
        welcome_icon.pack(pady=(15, 20))
        
        # Kullanıcı adını e-posta'dan çıkar
        username = email.split('@')[0] if '@' in email else email
        
        # Hoş geldin başlığı
        welcome_title = tk.Label(
            card_frame, 
            text=f"Hoş geldin, {username}!", 
            font=("Arial", 20, "bold"), 
            bg="white", 
            fg="#0077cc"
        )
        ...
        welcome_message = tk.Label(
            card_frame, 
            text="Bugün öğrenmeye hazır mısın?", 
            font=("Arial", 16), 
            bg="white", 
            fg="#444"
        )
        welcome_message.pack(pady=(0, 25))
        ...
        motivation_label.pack(pady=(0, 25))
        ...
        # Hover efekti
        def on_enter(e):
            start_btn.config(bg="#229954")
        def on_leave(e):
            start_btn.config(bg="#27ae60")
        ...
        def on_enter_close(e):
            close_btn.config(bg="#7f8c8d")
        def on_leave_close(e):
            close_btn.config(bg="#95a5a6")
        
        close_btn.bind("<Enter>", on_enter_close)
        close_btn.bind("<Leave>", on_leave_close)
        
        # ESC tuşu ile kapatma
        welcome_window.bind('<Escape>', lambda e: welcome_window.destroy())
        
        # X butonu ile kapatma
        welcome_window.protocol("WM_DELETE_WINDOW", on_close)
    ...
    def logout(self):
        """Çıkış yapar"""
        # Kayıtlı kullanıcı bilgilerini sil
    ...
    def show_topics(self):
        ...
        title_label.pack(pady=(40, 0))
        subtitle_label = tk.Label(self.root, text="DriveMate’in akıllı sistemiyle öğrenme artık sana özel. Hemen başla!", font=("Arial", 16), fg="#444", bg="#f0f0f0")
        subtitle_label.pack(pady=(0, 30))
    ...
    def show_topic_content(self, topic_id):
        ...
        topic_info = {
            1: {
                ...
                "system_prompt": """Senin Adın DriveMate. Sen, Türkiye'deki ehliyet adayları için yalnızca Trafik ve Çevre Bilgisi konularında uzmanlaşmış, sabırlı ve teşvik edici bir eğitmen olan Trafik ve Çevre Rehberi'sin.

                Amaç (Purpose): Temel amacın, kullanıcıların Türkiye'deki teorik ehliyet sınavının Trafik ve Çevre Bilgisi bölümüne eksiksiz bir şekilde hazırlanmalarını sağlamaktır.

                Talimatlar (Instructions - Düşünce Zinciri):

                Verdiğin yanıtlarda MUTLAKA AMA MUTLAKA 25 kelimeyi geçme.
                Kullanıcıyı sıcak bir şekilde karşıla ve kendini Drive Mate - Türkiye Trafik ve Çevre Bilgisi uzmanı olarak tanıt.
            ...
            4: {
                ...
                Verdiğin yanıtlarda mutlaka 25 kelimeyi geçme.
    ...
    def show_chatbot_interface(self, topic):
        ...
        def on_enter_key(event):
            # Shift+Enter ise yeni satır ekle
            if event.state & 0x1:  # Shift tuşu basılı
                return  # Normal davranışa devam et (yeni satır)
            else:
                # Sadece Enter ise mesaj gönder
        ...
        self.add_message_to_chat("DriveMate", welcome_msg, "#0077cc")
    ...
    def send_message(self, topic):
        ...
        self.add_message_to_chat("DriveMate", "Yazıyor... ⏳", "#999")
    ...
    def get_ai_response(self, user_message, topic):
        ...
        try:
            ...
            self.chat_history.append({"role": "DriveMate", "content": ai_response})
    ...
    def update_chat_with_response(self, response):
        ...
        # AI yanıtını ekle
        self.add_message_to_chat("DriveMate", response, "#0077cc")
    ...
    def show_level_test(self):
        ...
        # Test açıklaması
        desc_label = tk.Label(self.root, text="20 soruluk bu testle seviyeni keşfet!\nSorular, Gemini AI tarafından sana özel olarak hazırlandı.", font=("Arial", 16), fg="#444", bg="#f0f0f0")
    ...
    def show_test(self, questions, title, finish_callback, has_timer=False):
        ...
        if has_timer:
            ...
            timer_frame.pack(side="top", pady=(0, 10))
